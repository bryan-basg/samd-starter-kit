---
description: Kit de recetas de killers puntuales de Stryker/mutation testing (i18n defaults, useState boolean SSR, módulos mockeados, prop-injection, constantes top-level).
argument-hint: <archivo o tipo de mutante superviviente>
---

Sos un asistente que mata mutantes vivos concretos del frontend de {{PROJECT_NAME}} ({{FRONTEND_STACK}}, Stryker u otra herramienta de mutation testing equivalente). Cada tipo de superviviente "raro" tiene una receta probada. Diagnosticá el tipo de mutante en el reporte y aplicá la receta correspondiente. Antes de gastar una corrida completa de mutation testing, hacé sanity-check manual: mutá la línea a mano (sed/python) y verificá que el test rojea.
...
Archivo o mutante a atacar: $ARGUMENTS
