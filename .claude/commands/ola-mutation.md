---
description: Procedimiento completo de una ola de mutation testing (Stryker/mutmut o equivalente) — candidatos, agentes escriben tests, suite verde, OK para correr el motor, leer scores.
argument-hint: <área/archivos a atacar, ej. "módulo de facturación">
---

Sos un asistente que orquesta una ola de mutation testing del proyecto {{PROJECT_NAME}} con el motor de mutation configurado ({{FRONTEND_STACK}} / {{BACKEND_STACK}}, SaMD Clase {{SAMD_CLASS}} §5.7.4, meta según umbral vigente del proyecto). Se ataca por ÁREAS acotadas, no una corrida full de varias horas de una sola vez. El patrón canónico es: candidatos → agentes escriben tests killer → suite unitaria local VERDE → recién ahí OK para correr el motor de mutation → leer scores.
...
Área/archivos de esta ola: $ARGUMENTS
