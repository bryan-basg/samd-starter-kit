---
name: arrancar-stack-local
description: Levanta el stack local completo (emuladores/servicios de {{CLOUD_STACK}} + backend {{BACKEND_STACK}} + frontend {{FRONTEND_STACK}}) y siembra usuarios canónicos con login funcional. Úsala cuando el usuario pida arrancar/levantar/prender el entorno de desarrollo local, o cuando necesites el stack corriendo (emuladores + backend + frontend) para reproducir un bug, probar un cambio o grabar una captura.
---

# arrancar-stack-local — levantar el entorno de desarrollo local

Levanta el entorno de desarrollo local de {{PROJECT_NAME}} en orden, con login funcional, tal como quedó validado la última vez que alguien del equipo lo levantó desde cero.

## Cuándo usarla

- El usuario pide arrancar/levantar/prender el stack local (emuladores + backend + frontend).
- Necesitás reproducir un bug o probar un cambio end-to-end y el stack no está corriendo.
- Vas a grabar una captura o hacer una verificación manual que requiere login funcional.
- El login local falla o los usuarios de prueba no existen/no tienen la password esperada.
