---
name: diagnostico-prod-no-se-ve
description: Diagnostica el clásico "hice un cambio y no se aplica en prod / se borran cosas" — verifica deploy, commits sin pushear y caché del Service Worker antes de teorizar. Úsala cuando el dueño reporte que un cambio "no se ve", "no se aplicó", "se borran efectos" o que el cliente web y el cliente nativo empaquetado "no coordinan", antes de sospechar de un bug de código nuevo.
---

# diagnostico-prod-no-se-ve — Diagnóstico de "el cambio no se ve en prod"

Es el diagnóstico para cuando {{OWNER}} reporta que "un cambio no se aplicó en producción", "se borran efectos" o "el cliente web y el cliente nativo no coordinan" en el proyecto {{PROJECT_NAME}}. Esto NO es desorden aleatorio: tiene causas concretas y conocidas. Hay que verificarlas en orden ANTES de teorizar sobre bugs de código nuevo.

## Cuándo usarla

- {{OWNER}} dice que un cambio "no se aplicó" o "no se ve" en producción después de un deploy.
- Reporta que "se borran efectos" o aparecen cosas rotas/mezcladas (HTML nuevo con assets viejos).
- Dice que el cliente web y el cliente nativo empaquetado (app tipo Capacitor/Cordova) "no coordinan" o muestran versiones distintas.
- Antes de asumir que hay un bug de código nuevo cuando el síntoma es "no cambió nada" tras un push.
...
Corré el checklist de diagnóstico en este orden:
