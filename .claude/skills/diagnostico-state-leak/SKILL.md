---
name: diagnostico-state-leak
description: Diagnostica flakys/state-leak entre tests — falla en suite full pero pasa aislado; bisección, orden fijo, sonda contaminante→víctima, error primario. Usala cuando un test o archivo flakea de forma intermitente, cuando la suite falla distinto en cada corrida, o cuando estés por tocar un fixture global de tests para "arreglar" un flaky.
---

# diagnostico-state-leak — Diagnóstico de flakys por contaminación de estado entre tests

Diagnostica un flaky de tests que se sospecha es state-leak (backend o frontend) en el proyecto. NO "arreglar" el fixture global sin reproducir primero — hay que verificar la hipótesis con una sonda antes de tocar un fixture que corre sobre miles de tests.

## Cuándo usarla

- Un test o archivo de test flakea: falla quejándose en la suite completa pero pasa si se corre aislado.
- Cada corrida full de la suite falla tests DISTINTOS (una vez un archivo, otra vez otro, otra vez un grupo de componentes).
- "La primera mitad pasa" y "la segunda mitad pasa" por separado, pero corridas juntas fallan.
- Antes de modificar un fixture global de tests para intentar "arreglar" un flaky — primero hay que confirmar que realmente es state-leak y no un bug semántico real.
...
2. **Bisección por bloques contiguos de archivos + 1 víctima conocida**: acotar dónde está el envenenador. Si el runner reparte tests entre workers, recordar que distintas estrategias de reparto (por archivo, por scope, aleatorio) exponen leaks distintos.

3. **Sonda de 2 tests (contaminante→víctima)** ANTES de tocar cualquier fixture: confirmar que ese par reproduce el leak. Bisección rápida: renombrar/excluir el archivo sospechoso (p.ej. `archivo.test.ext.bak`) y re-correr la suite para confirmar si es el contaminante.
