...
- **11 workflows de CI/CD funcionales** (`.github/workflows/`) — gates de CI, auditoría de seguridad (Trivy + Semgrep), mutación nocturna (Stryker), fuzz del contrato de API (schemathesis), DAST (OWASP ZAP), SBOM, tier Postgres, drift del contrato OpenAPI, auditoría del estado del proyecto, triage de PRs viejos y una plantilla de deploy.
